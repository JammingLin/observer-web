...
-module(observer_sys_web).

-export([sys_info/1]).

sys_info(Node) ->
	SysInfo = observer_web:try_rpc(Node, observer_backend, sys_info, []),
	AllocInfo = proplists:get_value(alloc_info, SysInfo, []),
	AllocFields = alloc_info(AllocInfo, [], 0, 0, true),
    {Info, Stat} = info_fields(),
    InfoFields = observer_web_lib:fill_info(Info, SysInfo),
    StatFields =  observer_web_lib:fill_info(Stat, SysInfo),
