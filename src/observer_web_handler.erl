...
do_process(get_sys, Node) ->
	{Info, Stat, Alloc} = observer_sys_web:sys_info(Node),
...
do_process(get_perf, {Node, Type}) ->
	Data0 = observer_perf_web:perf_info(Node, Type),
	case Type of
		scheduler ->
...
wrap_info2(info, [{Name, Value}|Stat], Data) ->
	wrap_info2(info, Stat, [{obj, [{name, list_to_binary(Name)}, {value, list_to_binary(observer_web_lib:to_str(Value))}]} | Data]).
